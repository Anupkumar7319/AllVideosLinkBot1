...
async def admin_post(client, message: Message):
    ...
    for user in users_collection.find():
        ...
        try:
            ...
            await asyncio.sleep(1)  # ðŸ” Delay between each message
...
# âœ… 5. Callback Query Handler for Stats
...
# âœ… 6. Flask Webhook Root Check
@web_app.route("/", methods=["GET"])
def root():
    return "Bot is running!", 200

# âœ… 7. Auto-forward any user message to all channels
...
async def auto_forward_handler(client, message: Message):
    print("âš™ï¸ Auto-forward handler triggered")
    ...
    if message.text:
        ...
        print("âš ï¸ Unsupported message type")
        return

    post_data["messages"] = {}
    posts_collection.insert_one(post_data)
    print(f"ðŸ“¦ Saved post to MongoDB: {post_data['type']}")
    ...
    for channel_id in CHANNELS_ID:
        try:
            ...
            print(f"âœ… Auto-forwarded to {channel_id}")

        except Exception as e:
            print(f"âŒ Auto-forward failed to {channel_id}: {e}")

# âœ… 8. Admin Command to Re-send All MongoDB Posts to Channel(s)
...
async def resend_all_posts(client, message: Message):
    ...
    if not saved_posts:
        await message.reply("âš ï¸ No posts found in database.")
        return
            ...
            for channel_id in CHANNELS_ID:
                try:
                    if post_type == "text":
                        ...
                        print(f"âš ï¸ Skipping post due to missing file_id or unknown type: {post}")
                        continue
                    count += 1
                except Exception as e:
                    print(f"âŒ Failed to send to {channel_id}: {e}")

        except Exception as e:
            print(f"âŒ Error processing post: {e}")

    await message.reply(f"âœ… Re-sent {count} post(s) to all channels/groups.")
    
            

# âœ… 9. Run Flask + Pyrogram Client
if __name__ == "__main__":
    try:
        ...
        print("ðŸ” Restarting fallback backup_bot.py...")
